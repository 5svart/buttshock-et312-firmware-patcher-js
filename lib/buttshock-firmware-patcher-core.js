...
class ButtshockFirmwarePatcherCore {
  ...
  encrypt() {
    ...
    let encrypted_firmware = new Array(ButtshockFirmwarePatcherCore.FIRMWARE_LENGTH).fill(0);
  ...
  decrypt() {
    let funcs = [(x) => { return ((x ^ 0x62) + 0x41) & 0xff;},
                 ...
                 (x) => { return x; }];
    let iv = ButtshockFirmwarePatcherCore.IV;
    let decrypted_firmware = new Array(ButtshockFirmwarePatcherCore.FIRMWARE_LENGTH).fill(0);
    this.firmware.map((byte) => {
