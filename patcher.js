...
class ButtshockFirmwarePatcherCore {
  // TODO Track state of encrypted/decrypted/patched/etc when coming from a
  ...
  generate_checksum() {
    ...
    for (let i = 0; i < ButtshockFirmwarePatcherCore.FIRMWARE_LENGTH - 16; i = i + 1) {
      xor = xor ^ this.firmware[i];
      add = add + this.firmware[i];
    }
  ...
  encrypt() {
    ...
    this.firmware = encrypted_firmware;
    return this;
  }
  ...
  decrypt() {
    ...
    this.firmware = decrypted_firmware;
    return this;
  }
  ...
  patch(patches) {
    ...
    this.firmware = modified_fw;
    // Update the checksum so that we come out of the patching function with a
    // valid firmware
    let checksum = this.generate_checksum();
    for (let i = 0; i < 3; ++i) {
      this.firmware[this.firmware.length-16 + i] = checksum[i];
    }
    return this;
  }
}
